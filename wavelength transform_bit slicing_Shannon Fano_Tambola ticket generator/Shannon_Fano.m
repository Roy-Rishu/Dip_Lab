...
p=p(p>0);                     
%Finding frequency of each gray level intensity.
%Normalizing histogram counts into 
%Extracting only those intensity values that 
%Removing all zero probability gray levels.
[p_sorted,idx]=sort(p,'descend');   

symbols_sorted=symbols(idx);        

%Sorting probabilities from highest to 
%Rearranging symbols in the same sorted 
...
function codes=shannon_fano(symbols,p,codes,startIdx,endIdx) %shannon fano 
if startIdx>=endIdx
return;
end
